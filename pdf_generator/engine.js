...
import { hexToRgb, inchesToPoints, getStatusIcon, capitalizeFirst } from './utils.js';
export class TemplateEngine {
    ...
    async render(context) {
        return new Promise((resolve, reject) => {
            try {
                ...
                doc._customLeftMargin = leftMargin;
                doc._customRightMargin = inchesToPoints(layoutConfig.right_margin);
                doc._customTopMargin = topMargin;
                doc._customBottomMargin = inchesToPoints(layoutConfig.bottom_margin);
    ...
    async renderSections(doc, context) {
    ...
    async renderSection(doc, section, context) {
        ...
        if (this[rendererName]) {
            await this[rendererName].call(this, doc, section, context);
        }
        else {
            console.warn(`Unknown section type: ${sectionType}`);
        }
    }
    async _renderHeader(doc, section, context) {
    ...
    async _renderSummary(doc, section, context) {
    ...
    async _renderFramework(doc, section, context) {
    ...
    async _renderServices(doc, section, context) {
    ...
    async _renderBuildConfig(doc, section, context) {
    ...
    async _renderDeploymentDocs(doc, section, context) {
    ...
    async _renderEnvironmentVars(doc, section, context) {
    ...
    async _renderRecommendations(doc, section, context) {
    ...
    async _renderPageBreak(doc, section, context) {
        doc.addPage();
        // Reset position with custom margins after page break
        const leftMargin = doc._customLeftMargin || inchesToPoints(0.75);
        const topMargin = doc._customTopMargin || inchesToPoints(0.75);
        doc.x = leftMargin;
        doc.y = topMargin;
    }
}
