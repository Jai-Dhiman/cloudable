...
export class PDFGeneratorService {
  ...
  constructor(templatesDir?: string) {
    if (!templatesDir) {
      ...
      // Try multiple locations (order matters - most likely first)
      const possiblePaths = [
        join(cwd, 'pdf_generator', 'templates'), // Source templates (works for both dev and prod)
        join(currentDir, 'templates'), // If templates copied to dist
        join(currentDir, '..', 'pdf_generator', 'templates'), // Relative fallback
        join(cwd, 'dist', 'pdf_generator', 'templates'), // Production with templates copied
      ]
  ...
  listAvailableTemplates(): Array<{id: string; name: string; description: string; version: string}> {
  ...
  private createContext(analysis: ProjectAnalysis): TemplateContext {
