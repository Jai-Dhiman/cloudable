...
export default class Analyze extends Command {
  static description = 'Analyze AWS costs for your deployment with intelligent monitoring and human-in-the-loop control';
  ...
  async run(): Promise<void> {
    ...
    const recipientEmail = flags['demo-email'] || process.env.DEMO_EMAIL || 'demo@cloudable.dev';

    // Display header
    this.log('\n' + boxen(
      chalk.bold.cyan('📊 Cloudable Cost Analysis\n\n') +
      chalk.white('Deployment: ') + chalk.cyan(deploymentId) + '\n' +
      chalk.white('Mode: ') + chalk.gray(demoMode ? 'Demo' : 'Production'),
      {
        padding: 1,
    ...
    try {
      ...
      const analysisResult = await costService.generateCostAnalysis(deploymentId);
      const duration = ((Date.now() - startTime) / 1000).toFixed(1);

      // Update box to complete
      this.clearLines(8);
      this.log(this.createAgentBox(
        '📊 Cost Analysis Agent',
      ...
      const formattedReport = ReportFormatter.formatForPDF(analysisResult, deploymentId);
      ...
      this.log('\n' + boxen(
        ...
        chalk.white('📧 Report sent to: ') + chalk.cyan(recipientEmail) + '\n' +
        chalk.white('💰 Current week: ') + chalk.green(`$${analysisResult.lastWeekCost.totalCurrentWeek.toFixed(2)}`) + '\n' +
        chalk.white('📈 Monthly projection: ') + chalk.yellow(`$${analysisResult.expectedMonthlyCost.projected.toFixed(2)}`) + '\n' +
        chalk.white('💡 Potential savings: ') + chalk.green(`$${analysisResult.redFlagSummary.totalPotentialSavings.toFixed(2)}/month`) + '\n\n' +
      ...
      if (waitForReply) {
        await this.monitorForReplies(threadId, analysisResult, deploymentId, demoMode);
      }

    } catch (error) {
      this.error(`Cost analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private createAgentBox(
    ...
    const width = 80;
    const statusIcon = state === 'complete' ? '✅' : state === 'in_progress' ? '⏳' : '🔄';
    const statusColor = state === 'complete' ? chalk.green : state === 'in_progress' ? chalk.yellow : chalk.cyan;
    ...
    const lines = [
      topBorder,
      '│ ' + statusColor(titleLine) + ' '.repeat(Math.max(0, width - titleLine.length - 3)) + '│',
      '│ ' + chalk.gray(statusLine) + ' '.repeat(Math.max(0, width - statusLine.length - 3)) + '│',
    ];

    if (durationLine) {
      lines.push('│ ' + chalk.dim(durationLine) + ' '.repeat(Math.max(0, width - durationLine.length - 3)) + '│');
    } else {
  ...
  private displayCostSummary(report: any): void {
    this.log('\n' + boxen(
      ...
      chalk.cyan('Last Week: ') + chalk.white(report.costSummary.lastWeek.formatted) + '\n' +
      chalk.cyan('Previous Week: ') + chalk.white(report.costSummary.previousWeek.formatted) + '\n' +
      chalk.cyan('Change: ') + (report.costSummary.change.direction === 'up' ? chalk.red : chalk.green)(report.costSummary.change.formatted) + '\n' +
      chalk.cyan('Monthly Projection: ') + chalk.yellow(report.projections.monthly.formatted) + '\n\n' +
      ...
      chalk.bold('💡 Potential Savings: ') + chalk.green(report.redFlags.totalPotentialSavingsFormatted),
      {
    ...
    if (report.redFlags.items.length > 0) {
      ...
      report.redFlags.items.slice(0, 3).forEach((flag: any, index: number) => {
        const severityIcon = flag.severity === 'critical' ? '🚨' : flag.severity === 'warning' ? '⚠️' : 'ℹ️';
        const severityColor = flag.severity === 'critical' ? chalk.red : flag.severity === 'warning' ? chalk.yellow : chalk.blue;

        this.log(`  ${severityIcon} ${severityColor(flag.title)}`);
        this.log(`     ${chalk.gray(flag.description)}`);
        if (flag.estimatedSavingsFormatted) {
          this.log(`     ${chalk.green('Savings: ' + flag.estimatedSavingsFormatted)}`);
        }
        this.log('');
      });
    }
    ...
    if (report.learningInsights.length > 0) {
      ...
      report.learningInsights.slice(0, 3).forEach((insight: any) => {
        ...
        this.log(`    ${chalk.dim('Confidence: ' + insight.confidenceFormatted)}\n`);
      });
    }
  }
  ...
  private async monitorForReplies(
    ...
    try {
      ...
      const { getAgentMailClient } = await import('../integrations/agentmail.js');
      ...
      try {
        ...
        if (replies.length > 0) {
          ...
          for (const reply of replies) {
            ...
            this.log('\n' + boxen(
              ...
              chalk.white('From: ') + chalk.cyan(reply.context.from) + '\n' +
              chalk.white('Command: ') + chalk.yellow(displayCommand) + '\n\n' +
            ...
            const result = await commandProcessor.processEmailCommand({
              ...reply.context,
              report: this.convertToWeeklyCostReport(analysisResult, deploymentId)
            }, false); // Don't wait for confirmation in monitoring mode
            ...
            if (result.success) {
              this.log('\n' + boxen(
                ...
                chalk.white('Action: ') + chalk.cyan(result.command.intent.replace('_', ' ')) + '\n' +
                (result.actionResult?.resourceType ? chalk.white('Resource: ') + chalk.yellow(result.actionResult.resourceType) + '\n' : '') +
                (result.actionResult?.costImpact ?
                  chalk.white('Monthly Savings: ') + chalk.green(`$${result.actionResult.costImpact.monthlySavings.toFixed(2)}`) + '\n'
                  : '') +
            ...
            if (result.command.requiresConfirmation && !result.executed) {
              ...
              this.log(chalk.gray('Reply to the email with "yes" or "confirm" to proceed.\n'));
            }
          }
        ...
        if (error instanceof Error && error.message.includes('Timeout')) {
          ...
          this.log(chalk.gray('You can still reply to the email - it will be processed next time you run this command.\n'));
        } else {
      ...
      this.log(chalk.gray('Make sure AGENTMAIL_API_KEY is set in your environment.\n'));
    }
  }
   ...
   */
  private convertToWeeklyCostReport(analysisResult: any, deploymentId: string): any {
    return {
