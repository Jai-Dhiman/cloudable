...
import type { CostPrediction, MonthlyCostProjection } from './cost-projection-engine.js';
...
export class CostAnalysisService {
  ...
  constructor(config: CostAnalysisConfig = {}) {
    ...
    const hyperspellKey = config.hyperspellApiKey || (this.demoMode ? 'demo-key' : undefined);
    ...
    this.redFlagAggregator = new RedFlagAggregator(this.region, hyperspellKey, this.demoMode);
  ...
  async generateCostAnalysis(
    ...
    if (this.demoMode) {
      ...
      awsResources = this.demoDataGenerator.generateDemoResourceInventory(deploymentId);
    } else {
    ...
    const expectedNextWeekCost = await this.projectionEngine.predictNextWeek(historicalCosts);
    ...
    const { redFlags, summary: redFlagSummary } = await this.redFlagAggregator.detectAllRedFlags({
      deploymentId,
  ...
  private async getAWSResourceInventory(
    ...
    const [ec2Instances, rdsInstances, natGateways, s3Buckets] = await Promise.all([
    ...
    resources.push(...ec2Instances, ...rdsInstances, ...natGateways, ...s3Buckets);
  ...
  private async getEC2Resources(tags?: Record<string, string>): Promise<AWSResource[]> {
    const resources: AWSResource[] = [];
  ...
  private async getRDSResources(tags?: Record<string, string>): Promise<AWSResource[]> {
    const resources: AWSResource[] = [];
  ...
  private async getNATGatewayResources(tags?: Record<string, string>): Promise<AWSResource[]> {
    const resources: AWSResource[] = [];
        ...
        for (const natGateway of response.NatGateways) {
          if (natGateway.State === 'available') {
            ...
            resources.push({
              ...
              createdAt: natGateway.CreateTime?.toISOString() || new Date().toISOString(),
              monthlyCost: 32.85,
              metadata: {
  ...
  private async getS3Resources(tags?: Record<string, string>): Promise<AWSResource[]> {
    const resources: AWSResource[] = [];
      ...
      for (const service of costData.topServices.slice(0, 3)) {
        ...
        if (accuracy.sampleSize > 0) {
          insights.push({
            ...
            confidence: Math.max(0, 1 - Math.abs(accuracy.avgVariancePercent) / 100),
